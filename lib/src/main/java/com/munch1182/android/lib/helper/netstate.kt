...
class NetStateHelper : ARMUpdate by ARDefaultManager(), Releasable {
     ...
     */
    val curr
        @RequiresPermission(Manifest.permission.ACCESS_NETWORK_STATE) get() = runCatching { cm?.activeNetwork }.getOrNull()
    ...
    fun isNetAvailable(net: Network? = curr): Boolean {
        net ?: return false
        return getCapabilities(net)?.isNetAvailable() ?: false
    }

    @RequiresPermission(Manifest.permission.ACCESS_NETWORK_STATE)
    private fun getCapabilities(net: Network?): NetworkCapabilities? {
        return runCatching { net?.let { cm?.getNetworkCapabilities(it) } }.getOrNull()
    }
    ...
    fun hasTransport(net: Network? = curr, netType: NetworkType): Boolean {
        net ?: return false
        val cap = getCapabilities(net) ?: return false
    ...
    fun getNetworkType(net: Network? = curr): NetworkType {
        ...
        val capabilities = getCapabilities(net) ?: return NetworkType.None
