...
/**
 * 原意作为是否解释权限用途的判断，google任务权限应该在第一次被拒绝第二次被永久拒绝之前提醒
 * 所以只会在这个时机返回true，其余时机返回false，包括即请求前，被拒绝后返回false
 *
 * 但实践中有些权限需要请求之前解释（比如蓝牙相关的定位权限），还需要处理当权限永久拒绝后的设置跳转打开权限处理
 * 所以此方法也可以用于权限被永久拒绝的判断：即在请求后此方法返回true即被永久拒绝
 *
